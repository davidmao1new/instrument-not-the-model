"""Driver: run the prestige elicitation across every admitted model.

Starts and stops a llama-server per model, exactly as panel_gate.py does, and
calls probe_affiliation_prestige for each. Only ADMITTED models are elicited:
a covariate from a model that cannot tell a statistician from a retail associate
would be measuring nothing.

Resumable and stream-aware for the same reasons panel_gate.py is.

    .venv/Scripts/python.exe paper-a/src/run_prestige.py --backend vulkan --port 8080 \
        --threads 3 --only mistral-7b-instruct-v0.1,llama-2-7b-chat
"""
...
OUT_DIR = ROOT / "paper-a" / "data" / "prestige"
PROBE = ROOT / "paper-a" / "src" / "probe_affiliation_prestige.py"
...
def start_server(server, model_path, port, threads, backend, wait_s=900):
    ...
    while time.time() < deadline:
        ...
        try:
            if requests.get(f"http://127.0.0.1:{port}/health", timeout=3).json().get("status") == "ok":
                return proc
...
def main() -> int:
    ...
    admitted = [r["model"] for r in json.load(open(GATE, encoding="utf-8")) if r.get("admit")]
    wanted = {x.strip() for x in args.only.split(",")} if args.only else None
    todo = [m for m in admitted if wanted is None or m in wanted]

    print(f"[{args.backend}:{args.port}] prestige for {len(todo)} admitted model(s)", flush=True)

    for mid in todo:
        out = OUT_DIR / f"prestige_{mid}.jsonl"
        if out.exists() and out.stat().st_size > 0:
            print(f"[resume] {mid} already elicited, skipping", flush=True)
            continue
        ...
        try:
            ...
            continue
        try:
            subprocess.run([str(PY), str(PROBE), "--model-label", mid,
                            "--port", str(args.port)],
                           cwd=str(ROOT), check=False)
        finally:
            proc.kill(); proc.wait(timeout=30); time.sleep(3)

    print("\nprestige elicitation pass complete", flush=True)
    return 0
